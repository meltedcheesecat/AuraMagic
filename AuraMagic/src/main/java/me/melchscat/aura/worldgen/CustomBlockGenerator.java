...
public class CustomBlockGenerator extends ChunkGenerator {
    ...
    private static int AURA_START_BLOCK_ID = -1;
    ...
    private void generateAuraSpecificBlocks(GeneratedChunk chunk, int chunkX, int chunkZ) {
        ...
        if (AURA_GEM_ID == -1) AURA_GEM_ID = getBlockId(AuraMagic.getInstance().WindGemBlock);
        if (AURA_CRYSTAL_BLOCK_ID == -1) AURA_CRYSTAL_BLOCK_ID = getBlockId(AuraMagic.getInstance().WindCrystalBlock);
        if (AURA_CRYSTAL_LARGE_ID == -1) AURA_CRYSTAL_LARGE_ID = getBlockId(AuraMagic.getInstance().WindCrystalLarge);
        if (AURA_CRYSTAL_MEDIUM_ID == -1) AURA_CRYSTAL_MEDIUM_ID = getBlockId(AuraMagic.getInstance().WindCrystalMedium);
        if (AURA_CRYSTAL_SMALL_ID == -1) AURA_CRYSTAL_SMALL_ID = getBlockId(AuraMagic.getInstance().WindCrystalSmall);
        if (AURA_START_BLOCK_ID == -1) AURA_START_BLOCK_ID = getBlockId(AuraMagic.getInstance().AuraStartBlock);
        ...
        if (spawnArr.length > 0) {
            if ((spawnChunkX == chunkX) && (spawnChunkZ == chunkZ)) {
                ...
                // Start Aura Sign
                blockChunk.setBlock(spawnX, spawnY-1, spawnZ-3, AURA_START_BLOCK_ID, 0,0);
                // 2 Air blocks for his foot
                blockChunk.setBlock(spawnX, spawnY, spawnZ-4, 0, 0,0);
                blockChunk.setBlock(spawnX, spawnY-1, spawnZ-4, 0, 0,0);
            }
        }
    }
}
